...
const i18n = {
  ...
  translations: {
    'zh-TW': {
      ...
      'version': '版本',
      'close': '關閉',
      'version1.5.0.change1': '將備份系統從本地文件改為 IndexedDB，無需選擇文件位置',
      'version1.5.0.change2': '新增匯出/匯入 JSON 檔案功能，方便數據備份和遷移',
      'version1.5.0.change3': '優化備份機制：刪除操作立即備份，添加/更新操作使用防抖備份（10秒）',
      'version1.5.0.change4': '實作動態 SEO 優化，支援多語言 meta 標籤自動更新',
      'version1.5.0.change5': '改進數據同步邏輯，以 localStorage 為主，IndexedDB 為備份',
    ...
    'zh-CN': {
      ...
      'version': '版本',
      'close': '关闭',
      'version1.5.0.change1': '将备份系统从本地文件改为 IndexedDB，无需选择文件位置',
      'version1.5.0.change2': '新增导出/导入 JSON 文件功能，方便数据备份和迁移',
      'version1.5.0.change3': '优化备份机制：删除操作立即备份，添加/更新操作使用防抖备份（10秒）',
      'version1.5.0.change4': '实现动态 SEO 优化，支持多语言 meta 标签自动更新',
      'version1.5.0.change5': '改进数据同步逻辑，以 localStorage 为主，IndexedDB 为备份',
      'version1.4.1.change1': '全站支援多國語言（繁中、簡中、英文、日語）',
    ...
    'en': {
      ...
      'version': 'Version',
      'close': 'Close',
      'version1.5.0.change1': 'Changed backup system from local file to IndexedDB, no need to select file location',
      ...
      'version1.5.0.change4': 'Implemented dynamic SEO optimization with automatic multi-language meta tag updates',
      'version1.5.0.change5': 'Improved data sync logic: localStorage as primary, IndexedDB as backup',
    ...
    'ja': {
      ...
      'version': 'バージョン',
      'close': '閉じる',
      'version1.5.0.change1': 'バックアップシステムをローカルファイルから IndexedDB に変更、ファイル位置の選択不要',
      ...
      'version1.5.0.change4': '動的 SEO 最適化を実装、多言語 meta タグの自動更新をサポート',
      'version1.5.0.change5': 'データ同期ロジックを改善、localStorage をプライマリ、IndexedDB をバックアップとして使用',
